...
For instance,
%several, hybrid energy storage systems improve efficiency and lifetime without significant increase of the cost~\cite{Kim:JPS14}.  %\cite{Xie:TCAD13, Kim:JPS14}
~\cite{Lin:ICCAD14}
electric vehicles consume a non-negligible amount of battery energy for non-driving power components. Related study optimizes energy for heating, ventilation and air conditioning (HVAC) systems equipped in electric vehicles~\cite{Vatanparvar:DAC15}. %\cite{Vatanparvar:DAC15, Zhao:ICCAD15} 
%Electric vehicle battery packs consist of a large number of battery cells, and it is crucial to optimize the battery pack architectures in terms of capacity, cost and reliability~\cite{Steinhorst:ASPDAC16}. %\cite{Steinhorst:ASPDAC16, Wu:TCAD13}. 
%Electric vehicle battery management systems would better be able to accommodate multiple power sources, provide a good scalability and mitigate cell-to-cell variability. %\cite{Shin:TCAD15, Narayanaswamy:TVLSI17}
